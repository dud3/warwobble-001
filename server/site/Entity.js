...
Entity.prototype.setup = function (descr) {
    ...
    // this gives the action that the entitiy executes right now
    if (player.p[this.belongsTo].aggressive) {
        this.curAction = 7; // an aggressive players starts with all units
                            // in stand-ground-action, which actually makes
                            // for a very VERY VERY dumb AI already in its
                            // own right =)
    } else {
...
Entity.prototype.setdown = function (descr) {
    ...
    this.actN[20] = 49 - this.race;
    this.actN[21] = 51 - this.race;
    this.actN[22] = 53 - this.race;
    this.actN[23] = 57 - this.race;
    this.actN[24] = 59 - this.race;
    this.actN[25] = 73 - this.race;
    this.actN[26] = 63 - this.race;
    this.actN[27] = 65 - this.race;
    
    this.actN[28] = 67 - this.race;
    this.actN[29] = 71 - this.race;
    this.actN[30] = 77 - this.race;
    this.actN[31] = 79 - this.race;
    
    this.actN[33] = 55 - this.race;
    
    if (this.isBuilding) {
        this.processOrderAsMyself = this.processOrderAsBuilding;
    } else {
        this.processOrderAsMyself = this.processOrderAsUnit;
    }
};
...
Entity.prototype.updateCorpse = function(du) {
    this.corpseTime += du;
    
    return this.DELETED;
};

Entity.prototype.corpsify = function() {
    ...
    switch(this.orientation) {
        ...
        case 3:
            this.corpseSpriteX = 0;
            this.corpseSpriteY = 1;
            break;
        case 4:
        case 5:
            this.corpseSpriteX = 5;
            this.corpseSpriteY = 1;
            break;
        default:
            this.corpseSpriteX = 5;
...
Entity.prototype.processOrder = function(order) {
    if (order.name === 'inchp') { // used to heal and to attack
        this.hp += order.hp;
    }
    
    // send to the unit- or building-specific processOrder function
    this.processOrderAsMyself(order);
};
